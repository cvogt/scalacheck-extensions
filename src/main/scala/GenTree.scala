...
object GenTree{  
  ...
  */
  def tree[T]: Gen[T] = macro GenTreeMacros.tree[T]
  ...
  */
  def partialTree[T]: Gen[T] = macro GenTreeMacros.partialTree[T]
}

class GenTreeMacros(val c: reflect.macros.blackbox.Context){
  import c.universe._
      ...
      val gens = classes.collect{
        ...
        case s: ClassSymbol if !s.isAbstract => 
          ...
          if(paramLists.flatten.size > 22){
            ...
            q"$GenArities.resultOf((..$args) => new $s(...$idents))"
          } else {
